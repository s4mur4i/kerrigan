...
class sg(base):
    ...
    def start(self):
        ...
        parser = argparse.ArgumentParser(description='ec2 tool for devops', usage='''kerrigan.py sg <command> [<args>]
...
Second level options are:
    ...
    def report_sg(self):
        '''
        This function is a wrapper for parsing arguments and printing for sg attribute reports
        :return:
        '''
        logger.info("Started report generation command")
        a = awsrequests(session=self.account_information['session'])
        parser = argparse.ArgumentParser(description='report generation about sg', usage='''kerrigan.py sg report_sg [<args>]]
